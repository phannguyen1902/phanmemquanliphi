...
public class update_fess_details extends javax.swing.JFrame {
    ...
    public void update_TT_print() {
        ...
        try {
            ...
            txt_total_word.setText(rs.getString("totals_in_words"));
    ...
    public void update_data_add_fess() {
        try {
            ...
            String sql = "UPDATE fess_details SET student_name=?, roll_no=?, payment_mode=?, cheque_no=?, bank_name=?, "
                    + "dd_no =?,course_name=?, gstin=?, total_amount=?,date=? , amount=?, cgst=?, sgst=?, totals_in_words=?, remark=?, year1=?, year2=? where reciept_no=?";

            try {
                Connection con = databaseHelper.openConnection();
                PreparedStatement pstmt = con.prepareStatement(sql);
                pstmt.setString(1, t_add_fess.getStudent_name());
                pstmt.setString(2, t_add_fess.getRoll_no());
                pstmt.setString(3, t_add_fess.getPayment_mode());
                pstmt.setString(4, t_add_fess.getCheque_no());
                pstmt.setString(5, t_add_fess.getBank_name());
                pstmt.setString(6, t_add_fess.getDd_no());
                pstmt.setString(7, t_add_fess.getCourse_name());
                pstmt.setString(8, t_add_fess.getGstin());
                pstmt.setFloat(9, t_add_fess.getTotal_amount());
                java.util.Date utilDate = new java.util.Date(t_add_fess.getDate().getTime());
                pstmt.setDate(10, new java.sql.Date(utilDate.getTime()));
                pstmt.setFloat(11, t_add_fess.getAmount());
                pstmt.setFloat(12, t_add_fess.getCgst());
                pstmt.setFloat(13, t_add_fess.getSgst());
                pstmt.setString(14, t_add_fess.getTotal_in_words());
                pstmt.setString(15, t_add_fess.getRemark());
                pstmt.setInt(16, t_add_fess.getYear1());
                pstmt.setInt(17, t_add_fess.getYear2());
                pstmt.setInt(18, t_add_fess.getReciept_no());
