...
public class update_fess_details {
	...
	public boolean insert(TT_add_fess t_add_fess) throws Exception {
		String sql = "UPDATE fess_details SET reciept_no=?, student_name=?,roll_no=?, payment_mode=?,cheque_no=?,bank_name=?"
			+"dd_no =?,course_name=?,gstin=?,total_amount=?,date=?,amount=?,cgst=?,sgst=?,totals_in_words=?,remark=?,year1=?,year2=? date=?";
